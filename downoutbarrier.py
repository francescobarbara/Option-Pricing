# -*- coding: utf-8 -*-
"""
Created on Wed Jul  1 17:17:17 2020

@author: angus
"""


class DownOutBarrier (object):
    
    def __init__(self,T,S,B,payoff):
        self.time = T #no. of business days before expiry
        self.stock = S #ticker in yahoo finance
        self.barrier = B #barrier level of the option, below which option becomes worthless
        self.payoff = payoff #payoff is a function of stock at expiry time
    ...
    def Montecarlo_pricing(self, mu, sigma, intervals, simulations):
        ...
        for i in range(simulations) :
            ...
            for j in range(1,intervals):
                ...
                if l[j] <= self.barrier:
                    barrier = 'yes'
            estimated_payoff[i] = self.payoff(l[-1])
            if barrier == 'yes':
                estimated_payoff[i] = 0
